...
fh_extraction.setFormatter(logging.Formatter('%(asctime)s - %(levelname)s - %(message)s'))
...
def update_stats(success: bool, error_type: str = None):
    with stats_lock:
        ...
        if success:
            ...
            if error_type:
                extraction_stats['fail_types'][error_type] = extraction_stats['fail_types'].get(error_type, 0) + 1
...
def extract_text(url: str, session: requests.Session, rate_limiting_domains: list[str]) -> str:
    ...
    try:
        ...
        if "text/html" in content_type:
            ...
            try:
                ...
                update_stats(True)
                logger_extraction.info(f"Successfully extracted PDF text for URL: {url}")
                return pdf_text
            except Exception as e:
                error_msg = f"PDF extraction error: {str(e)}"
                update_stats(False, error_type=error_msg)
                logger_failures.error(f"Error extracting PDF text from {url}: {error_msg}")
                return f"PDF content from {url} (extraction failed)"
        else:
            update_stats(True)
            logger_extraction.info(f"Non-HTML/PDF content for URL: {url}. Using URL as fallback.")
            return url
    except Exception as e:
        error_msg = str(e)
        update_stats(False, error_type=error_msg)
...
def parallel_extraction(urls: list[str], max_workers: int, rate_limiting_domains: list[str]) -> list[str]:
    """Extract texts from URLs in parallel with progress indication."""
    texts = []
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(extract_text, url, create_session(), rate_limiting_domains): url for url in urls}
        for future in tqdm(as_completed(futures), total=len(futures), desc="Extracting texts"):
            texts.append(future.result())
    texts = [txt if txt and len(txt) > 50 else url for txt, url in zip(texts, urls)]
    return texts

def get_embeddings(urls: list[str], model_name: str = "all-MiniLM-L6-v2", max_workers: int = 10, rate_limiting_domains: list[str] = []):
    """Extract texts in parallel and compute embeddings."""
    texts = parallel_extraction(urls, max_workers=max_workers, rate_limiting_domains=rate_limiting_domains)
